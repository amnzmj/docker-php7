	...
	class DocumentosController extends Controller{
		...
		public function indexAction(){
			...
			$sql = "SELECT `vd`.* FROM `viewdocumentos` AS `vd`, `tblexpediente` AS `e` WHERE `vd`.`idExpediente` = `e`.`idExpediente` 
				AND `e`.`idAspirante` = " .  $identity['idAspirante']." ORDER BY documento ASC";
		...
		public function subirsubmitAction(){
			...
			if( move_uploaded_file($_FILES['archivo']['tmp_name'], $uploadPath . "/". $documento['idDocumento'] ) ){
				...
				if($cambioExt){
					...
					$checarCompletado = $db->fetchRow( "SELECT `ve`.* FROM `viewexpedientes` AS `ve` WHERE `ve`.`idExpediente` = " . $documento['idExpediente'] . " AND `cuentaSinDatos` = 0 AND `cuentaRechazados` = 0" );
					$checarpos= strtoupper(str_replace('_',' ',$checarCompletado['posgrado']));
                                        if($checarCompletado){
						...
						$dQuery = $db->fetchArray("SELECT `email`,`posgradoAdm` FROM `tbladministrador` WHERE `notificar` = 1 AND `activo` = 1 AND `posgradoAdm` = '$checarpos'");
